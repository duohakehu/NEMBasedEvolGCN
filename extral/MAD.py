...
class MAD:
    ...
    def init_OT_matrix(self):
        ...
        """
        cost_OT = torch.ones(size=(self.shape_X_source[1], self.shape_X_target[1]))
    ...
    def mat_cost_OT(self):
        ...
        """
        mat_cost = torch.zeros(size=(self.shape_X_source[1], self.shape_X_target[1]))
        if self.one_dim:
            for lab in range(0, self.num_class):
                ...
                C3 = torch.matmul(torch.matmul(self.X_source[self.tab_idx[lab], :, 0], pi_DTW[:]),
                                  self.X_target[:, :, 0].T)
                res = C1 + C2 - 2 * C3
            ...
            for lab in range(0, self.num_class):
                ...
                C3 = torch.tensordot(torch.matmul(self.X_source[self.tab_idx[lab]].transpose(1, -1), pi_DTW),
                                     self.X_target,
                                     dims=([1, 2], [2, 1]))
    ...
    def mat_dist_DTW(self, y_source_it):
        ...
        """
        if self.one_dim:
            if y_source_it is None or y_source_it == 0:
                OTc = self.OT
                X_source_c = self.X_source[:, :, 0]
            else:
                OTc = self.OT[self.tab_idx[y_source_it]]
                X_source_c = self.X_source[self.tab_idx[y_source_it], :, 0]
            C2 = torch.matmul(OTc.sum(axis=0).view(-1, 1), self.X_target_squared.view(1, -1))
            C3 = torch.matmul(torch.matmul(X_source_c, OTc), self.X_target.view(-1, 1))
            res = self.X_source_squared_sum[y_source_it].view(1, -1) + C2[:] - 2 * C3
        else:
    ...
    def fit(self, X_source, y_source=None, X_valid_source=None, y_valid_source=None, weights_X_source=None,
            previous_DTW=None):
        """
        ...
        if weights_X_source is None:
            self.X_source_a_one = torch.ones(size=(self.shape_X_source[1],)) / self.shape_X_source[1]
        else:
        ...
        for lab in range(0, self.num_class):
            ...
            if self.one_dim:
                ...
                X_source2_sum = torch.matmul(self.X_source_a_one[self.tab_idx[lab]].view(-1, 1), X_source2.view(1, -1))
            else:
    ...
    def run_for_output(self, X_target, weights_X_target=None):
        ...
        if weights_X_target is None:
            self.X_target_a_one = torch.ones(size=(self.shape_X_target[1],)) / self.shape_X_target[1]
        else:
            self.X_target_a_one = weights_X_target
